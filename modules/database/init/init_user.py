...
class UserCreator(ABC):
    ...
    def create_user_node(self, user_path, db_name, worker_node):
        ...
        try:
            return self._create_user_node(db_name, worker_node, user_path)
        except Exception as e:
            logging.error(f"Error creating user node: {e}")
            raise

    def _create_user_node(self, db_name, worker_node, user_path):
...
class SchoolUserCreator(UserCreator):
    ...
    def create_teacher_node(self, user_path):
        ...
        try:
            return self._create_teacher_node(user_path)
    ...
    # TODO Rename this here and in `create_teacher_node`
    def _create_teacher_node(self, user_path):
        teacher_node = entity_neo.TeacherNode(
            ...
            user_db_name=f"cc.ccusers.{format_user_email_for_neo_db(self.email)}",
            worker_db_name=f"cc.ccschools.{self.school_node.school_uuid}",
            path=user_path
        )
    ...
    def create_user_worker_relationship(self, user_node, worker_node):
        ...
        logging.info("Relationship created between user and worker")
    ...
    def create_worker_school_relationship(self, worker_node, school_node):
        ...
        logging.info("Relationship created between worker and school")
...
class NonSchoolUserCreator(UserCreator):
    ...
    def create_user_specific_relationship(self, user_node, specific_node):
        ...
        logging.info("Relationship created between user and specific node")
...
def create_user(db_name, user_type, username, email, user_id, school_node=None, worker_data=None):
    ...
    logging.info("[1.0] Starting user creation process")
    ...
    try:
        with neo4j_driver_tools.get_driver().session() as session:
            logging.info("[2.0] Creating user's private database")
    ...
    logging.info("[3.0] Initializing Neontology connection")
    neon.init_neontology_connection()

    # 3. Create filesystem structure for user's private database
    logging.info("[4.0] Setting up filesystem structure")
    ...
    if user_type in ['email_teacher', 'ms_teacher', 'email_student', 'ms_student']:
        logging.info("[5.0] Creating school-related worker nodes")
        if not school_node:
            logging.error("[5.X] School node required but not provided")
        ...
        logging.info("[5.2] Created school worker directory at: {school_worker_path}")
        ...
        if user_type in ['email_teacher', 'ms_teacher']:
            logging.info("[6.0] Creating teacher worker nodes")
            ...
            logging.info("[6.1] Created school teacher node: {school_worker_node.unique_id}")
            school_fs_handler.create_default_tldraw_file(school_worker_path, school_worker_node.to_dict())
            logging.info("[6.2] Created school tldraw file")
            ...
            logging.info("[6.3] Created private teacher node: {private_worker_node.unique_id}")
        else:
            logging.info("[6.0] Creating student worker nodes")
            # School DB worker node
            school_worker_node = create_student_node(
                school_db, user_id, username, email, 
                school_worker_path, worker_data, school_node
            )
            logging.info("[6.1] Created school student node: {school_worker_node.unique_id}")
            school_fs_handler.create_default_tldraw_file(school_worker_path, school_worker_node.to_dict())
            logging.info("[6.2] Created school tldraw file")
            ...
            logging.info("[6.3] Created private student node: {private_worker_node.unique_id}")
        private_fs_handler.create_default_tldraw_file(private_worker_path, private_worker_node.to_dict())
        logging.info("[6.4] Created private tldraw file")
        # Create relationships in school DB
        logging.info("[7.0] Creating school relationships")
        ...
        logging.info("[7.1] Created school relationship")

        # Use private worker node for user node creation
        worker_node = private_worker_node
    else:
        logging.info("[5.0] Non-school user type, skipping worker node creation")
        worker_node = None

    # 5. Create user node with worker data in private DB
    logging.info("[8.0] Creating main user node")
    user_node = entity_neo.UserNode(
        unique_id=f"User_{formatted_email}",
        user_id=user_id,
        user_type=user_type,
    ...
    logging.info("[8.1] Created user node: {user_node.unique_id}")

    # Create tldraw file for user node
    private_fs_handler.create_default_tldraw_file(user_private_path, user_node.to_dict())
    logging.info("[8.2] Created user tldraw file")

    # 6. Create relationship between user and worker in private DB
    if worker_node:
        logging.info("[9.0] Creating user-worker relationship")
        neon.create_or_merge_neontology_relationship(
            entity_relationships.UserIsWorker(source=user_node, target=worker_node),
            database=user_db_name,
            operation='merge'
        )
        logging.info("[9.1] Created user-worker relationship")

    # 7. Create calendar for user
    try:
        logging.info("[10.0] Creating user calendar")
        calendar_nodes = init_calendar.create_calendar(
            user_db_name,
            datetime(2024, 9, 1).date(),  # Start of school year 2024
            datetime(2025, 7, 31).date(),  # End of school year 2025
        ...
        logging.info("[10.2] Preparing calendar response")
        serialized_calendar = {
        ...
        logging.info("[11.0] User creation process complete")
        return {
        ...
        logging.error("[10.X] Error creating calendar: {str(e)}")
        logging.info("[11.0] User creation complete with calendar error")
        return {
            'user_node': user_node.to_dict(),
            'worker_node': worker_node.to_dict() if worker_node else None,
            'calendar_nodes': None
...
def create_teacher_node(db_name, user_id, username, email, path, worker_data, school_node):
    ...
    user_db_name = f"cc.ccusers.{format_user_email_for_neo_db(email)}"
